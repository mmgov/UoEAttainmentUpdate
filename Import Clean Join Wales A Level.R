...
View(WALevel2014to2017)

#Rename 2017 column

colnames(WALevel2014to2017)[colnames(WALevel2014to2017)=="Average wider points score for pupils aged 17"] <-"Average wider points score for pupils aged 17 2017"
...
schnonew <- WALevel2014to2017[is.na(WALevel2014to2017$`Average wider points score for pupils aged 17 2017`),]
...
#Import this updated sheet so the notes can be appended
  ...
  library(readr)
NIschnonewnotes<- read_csv("NIAlevelOldSchoolsNoNewData.csv")
...
WALevel2014to2017New <- WALevel2014to2017 %>% 
  filter( !is.na(`Average wider points score for pupils aged 17 2015`)&
            !is.na(`Average wider points score for pupils aged 17 2016`) &
            !is.na(`Average wider points score for pupils aged 17 2017`))
...
x <- WALevel2014to2017$`Average wider points score for pupils aged 17 2017`
x <-x[!is.na(x)]
mu <- mean(x,trim = 0)
totalvar <- sum((x-mu)^2)
...
WALevel2014to2017 <- WALevel2014to2017 %>% 
  mutate(`Z Score 2017`= (`Average wider points score for pupils aged 17 2017`- 
                            mean(`Average wider points score for pupils aged 17 2017`, na.rm=TRUE))/pop_sd)
...
WALevel2014to2017 <- WALevel2014to2017 %>% 
  mutate (`Z Score 2015-2017` = rowMeans(WALevel2014to2017[,c(9,11,13)], na.rm = TRUE))

#round to 2 decimal places


WALevel2014to2017 <- WALevel2014to2017 %>% 
  mutate(`Z Score 2015-2017`= round(WALevel2014to2017$`Z Score 2015-2017`,digits=2))
...
WALevel2014to2017$`Above or Below Average new`[WALevel2014to2017$`Z Score 2015-2017`<=0] <- "Below Average"
WALevel2014to2017$`Above or Below Average new`[WALevel2014to2017$`Z Score 2015-2017`>0] <- "Above Average"

View(WALevel2014to2017)


# What still needs to be done: Validate data 1) Validate Average, SD and Z score for each year by calculating the figures again another way 2) Volatility check, did the Z score change more than 2 (in either direction) between 2015 and 2016 or between 2016 and 2017?
